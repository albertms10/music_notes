...
final class IntervalClass
  ...
  /// Returns the [PitchClass] cycle for this [IntervalClass] starting from
  /// [origin].
  ///
  /// The cycle is formed by repeatedly transposing [origin] by this interval
  /// class until returning to [origin].
  ///
  /// See [Interval cycle](https://en.wikipedia.org/wiki/Interval_cycle).
  ///
  /// Example:
  /// ```dart
  /// IntervalClass.m3.cycle() == <PitchClass>{.c, .dSharp, .gSharp, .a}
  /// IntervalClass.M3.cycle() == <PitchClass>{.c, .e, .gSharp}
  /// IntervalClass.tritone.cycle(origin: .e) == <PitchClass>{.e, .bFlat}
  /// ```
  Set<PitchClass> cycle({PitchClass origin = .c}) {
    final cycle = <PitchClass>{};
    var current = origin;

    do {
      cycle.add(current);
      current = PitchClass(current.semitones + semitones);
