...
final class IntervalClass
  ...
  /// ```
  int get cycleLength =>
      chromaticDivisions ~/ semitones.gcd(chromaticDivisions);
  ...
  /// Returns all distinct [PitchClass] cycles generated by this
  /// [IntervalClass].
  ///
  /// ```dart
  /// IntervalClass.minorThird.cycles.toList() == <Set<PitchClass>>[
  ///   {.c, .dSharp, .fSharp, .a},
  ///   {.cSharp, .e, .g, .aSharp},
  ///   {.d, .f, .gSharp, .b},
  /// ]
  /// ```
  Iterable<Set<PitchClass>> get cycles sync* {
    final seen = <PitchClass>{};
    for (final origin in PitchClass.values) {
      if (seen.contains(origin)) continue;

      final cycle = this.cycle(origin: origin);
      seen.addAll(cycle);
