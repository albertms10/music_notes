...
final class IntervalClass
  ...
  /// A list of all [IntervalClass] values.
  static const values = <IntervalClass>[
    P1, m2, M2, m3, M3, P4, tritone, //
  ];

  /// The number of distinct [PitchClass] cycles generated by this
  /// [IntervalClass], derived directly from the semitones/[chromaticDivisions]
  /// ratio.
  ///
  /// This is the group index of the subgroup [cycle] generates: cosets of
  /// that subgroup in ℤ₁₂ are congruent mod this value, so it also equals
  /// [chromaticDivisions] divided by [cycleLength].
  ///
  /// Example:
  /// ```dart
  /// IntervalClass.P1.cycleCount == 12
  /// IntervalClass.tritone.cycleCount == 6
  /// IntervalClass.M3.cycleCount == 4
  /// IntervalClass.m3.cycleCount == 3
  /// ```
  int get cycleCount => semitones.gcd(chromaticDivisions);

  /// The number of distinct [PitchClass]es in the cycle generated by this
  /// [IntervalClass].
  ///
  /// Its value is derived directly from the semitones/[chromaticDivisions]
  /// ratio rather than by generating the cycle itself.
  ///
  /// Example:
  /// ```dart
  /// IntervalClass.P1.cycleLength == 1
  /// IntervalClass.tritone.cycleLength == 2
  /// IntervalClass.M3.cycleLength == 3
  /// IntervalClass.m3.cycleLength == 4
  /// ```
  int get cycleLength => chromaticDivisions ~/ cycleCount;

  /// The [PitchClass] cycle for this [IntervalClass] starting from [origin].
  ///
  /// The cycle is formed by repeatedly transposing [origin] by this interval
  /// class until returning to [origin]. Unlike a [Set], the returned
  /// [Iterable] is lazy and preserves generation order, and its length is
  /// known upfront via [cycleLength] without iterating.
  ///
  /// See [Interval cycle](https://en.wikipedia.org/wiki/Interval_cycle).
  ///
  /// Example:
  /// ```dart
  /// IntervalClass.m3.cycle().toSet() == <PitchClass>{.c, .dSharp, .gSharp, .a}
  /// IntervalClass.M3.cycle().toSet() == <PitchClass>{.c, .e, .gSharp}
  /// IntervalClass.tritone.cycle(origin: .e).toSet()
  ///   == <PitchClass>{.e, .bFlat}
  /// ```
  Iterable<PitchClass> cycle({PitchClass origin = .c}) =>
      .generate(cycleLength, (i) => .new(origin.semitones + semitones * i));

  /// All distinct [PitchClass] cycles generated by this [IntervalClass].
  ///
  ...
  /// Example:
  /// ```dart
  /// IntervalClass.m3.cycles.toList() == <Set<PitchClass>>[
  ///   {.c, .dSharp, .fSharp, .a},
  ///   {.cSharp, .e, .g, .aSharp},
  ///   {.d, .f, .gSharp, .b},
  /// ]
  /// ```
  Iterable<Set<PitchClass>> get cycles =>
      .generate(cycleCount, (origin) => cycle(origin: .new(origin)).toSet());
